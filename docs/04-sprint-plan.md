...
| **Sprint 7** | Account Lifecycle & Polish | Soft-delete account flow (60-day reactivation window), UI/UX polish, bug fixes, end-to-end testing on Web (local) |
...
## Local Development Stack (Sprints 0–7, before Azure is active)
...
- **Azure subscription is NOT required to start.** Sprint 0 through Sprint 7 run entirely on the local dev stack (see above); Azure comes in at Sprint 7.5 once the subscription is active.
...
- Use EF Core migrations from the start (even against SQLite/LocalDB) so the schema transfers cleanly to Azure SQL in Sprint 7.5 with no rework.
