...
Find the first test case with Status "Not Run", top to bottom. Skip anything already Passed/Failed/Blocked.
Execute ONLY that one test case via actual browser interaction at http://localhost:8080. No API-only shortcuts, no code reading to infer behavior — a real click-through using the browser subagent.
Reuse existing test data already recorded in this file (accounts, IDs, etc.) — do not create new throwaway data unless that specific test case explicitly requires fresh data (e.g. registration tests).
Update that test case's Status directly in this file: Passed, Failed, or Blocked.
Add or update an "Actual:" line under the Expected line with exactly what happened — real response bodies/status codes where relevant, what appeared on screen, and any new data created (so later tests can reuse it).
If it Failed, do not attempt to fix the underlying code — just record the failure accurately.
Commit with message "test: execute [TEST-ID] — [Passed/Failed]" and push to main.
Stop after this one test. Report back which test ran, what was done, what was observed, and the status set. Do not proceed to the next test until told to.
...
Expected: Indicator shows green immediately after landing on the dashboard — not red, not requiring a delay or refresh, and no transient flash to red at any point. 
...
5. PRESENCE
Note: The actual UI path for all tests below is: click your profile avatar → "Privacy Settings" section in the profile modal → visibility dropdown (see AM1/AM2 in ACCOUNT MANAGEMENT for the modal itself). Custom-mode exceptions are managed via the "Manage Exceptions" button in the same modal.
...
Steps: Set to "Custom," explicitly allow one specific non-connected account via "Manage Exceptions." 
Expected: Only that account can see presence; no one else. 
Status: Not Run

P5 — Custom: deny one connected user
Steps: Set to "Custom," explicitly deny one connected account via "Manage Exceptions." 
...
6. SEARCH
S1 — Search returns matching user
Steps: In the Contacts tab, search for an existing user's handle. 
Expected: The matching user appears in results with correct handle/phone shown. 
Status: Not Run

S2 — Search with no matches
Steps: Search for a handle that doesn't exist. 
Expected: Empty state shown, no error, no stale results from a previous search. 
Status: Not Run

S3 — Clearing search box clears results
Steps: Search for a user, get results, then manually delete all text from the search box. 
Expected: Results list disappears/clears automatically without needing to press Search again. 
Status: ❌ Confirmed bug. _searchUsers() returns early on an empty query without clearing _searchResults, and there's no onChanged listener on the search field to detect clearing. Stale results remain visible until a new search is submitted. Needs a fix: add onChanged to clear _searchResults when the field becomes empty.

S4 — Search result shows "Connected" state correctly
Steps: Search for a user you're already connected to. 
Expected: Shows a "Connected" badge instead of a "Send Request" button. Confirmed present in code (isConnected branch renders a green "Connected" badge). 
Status: Not Run

S5 — Search result shows "Accept/Decline" when they already sent you a request
Steps: Have another user send you a connect request, then search for that user. 
Expected: Shows "Accept" and "Decline" buttons instead of "Send Request". Confirmed present in code (hasPendingRequest branch). 
Status: Not Run

S6 — Search for yourself
Steps: Search for your own handle. 
Expected: Confirm actual behavior — decide if you should appear in your own results or be excluded. Not yet confirmed either way; may reveal a gap. 
Status: Not Run

S7 — Search for a blocked user (or a user who blocked you)
Steps: Search for a user in either blocking relationship. 
Expected: Confirm actual behavior — should they appear in results at all, and if so, should "Send Request" be disabled given B2 already blocks the request server-side regardless. 
Status: Not Run

7. ACCOUNT MANAGEMENT
AM1 — Presence settings accessible via profile modal
Steps: Click your profile avatar to open the profile modal. 
Expected: Modal opens showing handle, email, and a "Privacy Settings" section with the visibility dropdown (Everyone/My connections/Nobody/Custom). 
Status: Not Run

AM2 — Manage Exceptions dialog
Steps: With visibility set to "Custom" in the profile modal, click "Manage Exceptions." 
Expected: A dialog opens allowing you to add/remove specific users as allowed or denied exceptions. Cross-reference with P4/P5. 
Status: Not Run

AM3 — View account summary
Steps: Open the Settings screen (the dedicated Account Settings tab, separate from the profile modal). Check the "Account Summary" card. 
Expected: Correct User ID and Email shown for the logged-in account. 
Status: Not Run

AM4 — Soft-delete account
Steps: In Settings, scroll to "Danger Zone," click "Delete Account," confirm the dialog. 
Expected: Per the on-screen copy: account is flagged as soft-deleted with a 60-day recovery window. Confirm what actually happens immediately after confirming — are you logged out right away? 
Status: Not Run

AM5 — Reactivation on login within the 60-day window
Steps: After AM4, attempt to log back in with the same soft-deleted account's credentials within the recovery window. 
Expected: Per the UI's stated behavior ("Logging back in during this window automatically reactivates your account"), login should succeed and the account should be un-flagged as deleted. 
Status: Not Run

AM6 — Login attempt after reactivation window expires
Steps: This likely isn't practically testable without manipulating the deadline directly in the database — if so, mark as Blocked. If testable (e.g. by manually setting ReactivationDeadline to a past date in the local dev DB), attempt login and confirm actual behavior. 
Expected: Confirm what happens — full denial of login, or something else. 
Status: Not Run

8. CALL HISTORY (display only — no live call needed)
...
10. WEBRTC / CALLING (tracked here for completeness — see CONNECT_PROJECT_STATUS.md for full detail)
